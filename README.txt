How to train the model:
	1. The first step is the preprocessing of the data - use preprocessing.py to run this step
	2. Training the model - use DDPG_Model/Training.py to run this step
	3. The trained model is saved at models/*run time* 

The repo for this code:


We included some of the data.
To get the full movement dataset visit:
https://github.com/sealneaward/nba-movement-data
To get the full play-by-play dataset use:
https://github.com/swar/nba_api